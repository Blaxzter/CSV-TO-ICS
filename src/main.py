    ...
    for event_entry in csv_list:
        ...
        if is_whole_day:
            if event_entry.get('Endet um') == '00:00:00':
                e.end = parse_date(event_entry.get("Endet am"), event_entry.get('Endet um'), True, whole_day=is_whole_day)
            e.make_all_day()
...
csv_content = read_csv_file(file_name)
